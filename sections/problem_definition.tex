...
\begin{figure}[H]
  \center{\includegraphics[width=0.5\linewidth]{model_task.png}}
  \caption{Mathematical model of a task.}
  \label{automaton}
\end{figure}

The worst-case execution time \( U_{i} \), representing the maximum fraction of CPU time spent to execute the task \( \tau_{i} \), can be calculated as follows:
\begin{equation*}
  U_{i} = \ceil[\bigg]{\frac{c_{i,\,j}}{T_{i}}}
\end{equation*}

and the total system utilization \( U \) is given by
\begin{equation*}
  U = \displaystyle\sum_{i} U_{i}
\end{equation*}
